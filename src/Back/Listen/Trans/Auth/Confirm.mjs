/**
 * Process confirmations for fronts authentication.
 *
 * @namespace TeqFw_Web_Event_Back_Listen_Trans_Auth_Confirm
 */
export default class TeqFw_Web_Event_Back_Listen_Trans_Auth_Confirm {
    constructor(spec) {
        // DEPS
        ...
        /** @type {TeqFw_Web_Event_Shared_Event_Front_Auth_Confirm} */
        const esfConfirm = spec['TeqFw_Web_Event_Shared_Event_Front_Auth_Confirm$'];
        ...
        const modRegistry = spec['TeqFw_Web_Event_Back_Mod_Registry_Stream$'];
        /** @type {typeof TeqFw_Web_Event_Back_Enum_Stream_State} */
        const STATE = spec['TeqFw_Web_Event_Back_Enum_Stream_State$'];

        // MAIN
        logger.setNamespace(this.constructor.name);
        eventsBack.subscribe(esfConfirm.getEventName(), handler);
        ...
        async function handler({data, meta}) {
            ...
            try {
                ...
                const stream = modRegistry.get(streamUuid, false);
                if (stream) {
                    stream.state = STATE.ACTIVE
                    clearTimeout(stream.unauthenticatedCloseId); // remove 'Not authenticated' timer
                    // re-publish delayed events
                    // portalFront.sendDelayedEvents(data.frontId, frontUuid).then();
        ...
        async function onResponse({data, meta}) {
            ...
            if (encrypted) {
                ...
                if (stream) {
                    ...
                    const serverSecret = modServerKey.getSecret();
                    ...
                    try {
                        ...
                        if (decrypted === backUUID.get()) {
                            ...
                            event.meta.streamUuid = frontUuid;
